...
X_train['int.rate'] = [x.strip('%') for x in X_train['int.rate']]
X_train['int.rate'] = X_train['int.rate'].astype(float)
X_train['int.rate'] = [x/100 for x in X_train['int.rate']]

X_test['int.rate'] = [x.strip('%') for x in X_test['int.rate']]
X_test['int.rate'] = X_test['int.rate'].astype(float)
X_test['int.rate'] = [x/100 for x in X_test['int.rate']]
...
for i in cat_df.columns :
    ...
    X_test.fillna(value='NA', inplace=True)
...
dot_data = export_graphviz(p_tree.best_estimator_, out_file=None, feature_names=X_train.columns, filled=True, class_names=['loan_paid_back_yes','loan_paid_back_no'])
