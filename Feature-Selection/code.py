...
frame = { 'Features' : Features, 'scores' : scores }
...
#scaled_features_train_df[[c for c in scaled_features_train_df.columns if c in top_k_predictors]]

scaled_features_train_df = scaled_features_train_df[[c for c in scaled_features_train_df.columns if c in top_k_predictors]]

model_fit_top_features = clf.fit(scaled_features_train_df, Y_train)

scaled_features_test_df = scaled_features_test_df[[c for c in scaled_features_test_df.columns if c in top_k_predictors]]

predictions_top_features = clf.predict(scaled_features_test_df)
